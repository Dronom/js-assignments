...
function getAverage(value1, value2) {
    return ((value1/10000 + value2/10000)*5000);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let xy = x1 * x2 + y1 * y2;
    let a = Math.pow(x1,2) + Math.pow(x2,2);
    let b = Math.pow(y1,2) + Math.pow(y2,2);
    if(Math.sqrt(a*b) === 0) return Math.acos(xy);
    return Math.acos(xy/(Math.sqrt(a*b)));
}
